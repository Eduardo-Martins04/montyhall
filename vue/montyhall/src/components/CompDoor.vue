<template>
  <div class="door-area">
    <div class="door-frame" :class="{selected: selected && !open}">
        <CompGift  v-if="open && hasGift"/>
    </div>
    <div class="door" :class="{open}"
        @click="selected = !selected">
        <div class="number" :class="{selected}">{{number}}</div><!--select: select == true -->
...
<script>
import CompGift from './CompGift.vue'

export default {
    name: 'CompDoor',
    components: { CompGift },
    props: { /*Propriedas de criação/instanciação da porta, não sera mudadas durante a execução*/
        number: {},
        hasGift: {type: Boolean}
    },
    data: function(){/* data ficam variaveis que poderam ser alteradas durante a execução do programa*/
        return {
            open: false,
            selected: false
        }
